...
JOIN <- right_join(prop %>% select(API, PROPNUM, WELLCOMPID, LEASE, BUDGET_NODE, TYPE_CURVE_REGION, 
                   ...
                   daily %>% 
                     ...
                     rename(GRGAS_Mcf_Prod = Vol_Gas_Prod_Mcf, 
                            GROIL_Bbl_Prod = Vol_Oil_Prod_Bbl, 
                            GRGAS_Mcf_Sales = Vol_Gas_Sales_Mcf, 
                            GROIL_Bbl_Sales = Vol_Oil_Sales_Bbl) %>%
                     select(Aries_Prop_Num, Prod_Dt, MON_END_DATE, GRGAS_Mcf_Prod, GROIL_Bbl_Prod,
                            GRGAS_Mcf_Sales, GROIL_Bbl_Sales),
  ...
  mutate(prod = 1, 
         ...
         C2_Shrink_Mcf = GRGAS_Mcf_Sales * C3_Sales * C3_Shrink,
         C3_Shrink_Mcf = GRGAS_Mcf_Sales * C3_Sales * C3_Shrink,
         iC4_Shrink_Mcf = GRGAS_Mcf_Sales * iC4_Sales * iC4_Shrink,
         nC4_Shrink_Mcf = GRGAS_Mcf_Sales * nC4_Sales * nC4_Shrink,
         iC5_Shrink_Mcf = GRGAS_Mcf_Sales * iC5_Sales * iC5_Shrink,
         nC5_Shrink_Mcf = GRGAS_Mcf_Sales * nC5_Sales * nC5_Shrink,
         C6Plus_Shrink_Mcf = GRGAS_Mcf_Sales * C6Plus_Sales * C6Plus_Shrink,
         ...
         GRGAS_Mcf_Shrunk_w_Ethane = GRGAS_Mcf_Sales - Total_Shrink_Mcf_w_Ethane,
         GRGAS_Mcf_Shrunk_wo_Ethane = GRGAS_Mcf_Sales - Total_Shrink_Mcf_wo_Ethane,
         NETGAS_Mcf = GRGAS_Mcf_Sales *  NetRevenueInterest / 100,
         NETOIL_Bbl = GROIL_Bbl_Sales * NetRevenueInterest / 100,
         NETGAS_Mcf_Shrunk_w_Ethane = NETGAS_Mcf - Total_Shrink_Mcf_w_Ethane,
         NETGAS_Mcf_Shrunk_wo_Ethane = NETGAS_Mcf - Total_Shrink_Mcf_wo_Ethane,
         NETNGL_Bbl_w_Ethane = GRGAS_Mcf_Sales / 1000 * NetRevenueInterest / 100 * NGLYield_Sales_w_Ethane,
         NETNGL_Bbl_wo_Ethane = GRGAS_Mcf_Sales / 1000 * NetRevenueInterest / 100 * NGLYield_Sales_wo_Ethane,
         NETBOE_Prod_w_Ethane = NETOIL_Bbl + NETGAS_Mcf_Shrunk_w_Ethane / 6 + NETNGL_Bbl_w_Ethane, 
         NETBOE_Prod_wo_Ethane = NETOIL_Bbl + NETGAS_Mcf_Shrunk_wo_Ethane / 6 + NETNGL_Bbl_wo_Ethane) %>% 
  left_join(., fcube %>%
              filter(ScenarioName == "Budget" | ScenarioName == "Forecast") %>% 
              select(ProductionMonthEndDate, ScenarioName, NetSalesGasMCF, NetSalesOilBBLs, NetSalesNGLBBLs, 
                     BudgetNode), by = c("MON_END_DATE" = "ProductionMonthEndDate"))




###aggregate to a month level====================================================================================
JOIN <- right_join(prop %>% select(API, PROPNUM, WELLCOMPID, LEASE, BUDGET_NODE, TYPE_CURVE_REGION, 
                                   TYPE_CURVE_SUBREGION, LATITUDE ,LATITUDE_BH, LONGITUDE, 
                                   LONGITUDE_BH, FIRST_PROD, EFF_LAT, RSV_CAT), 
                   daily %>% 
                     mutate(MON_END_DATE = ceiling_date(Prod_Dt, "month") - days(1)) %>% ##time zero
                     group_by(Aries_Prop_Num, MON_END_DATE) %>%
                     arrange(Aries_Prop_Num, MON_END_DATE) %>%
                     summarise(GRGAS_Mcf_Prod = sum(Vol_Gas_Prod_Mcf, na.rm = TRUE), 
                               GROIL_Bbl_Prod = sum(Vol_Oil_Prod_Bbl, na.rm = TRUE), 
                               GRGAS_Mcf_Sales = sum( Vol_Gas_Sales_Mcf, na.rm = TRUE), 
                               GROIL_Bbl_Sales = sum(Vol_Oil_Sales_Bbl, na.rm = TRUE)) %>%
                   select(Aries_Prop_Num, MON_END_DATE, GRGAS_Mcf_Prod, GROIL_Bbl_Prod,
                          GRGAS_Mcf_Sales, GROIL_Bbl_Sales),
                   by = c("PROPNUM" = "Aries_Prop_Num")) %>%
  ##join well interest table
  left_join(., interest %>%
  ...
  mutate(prod = 1, 
         ...
         NETBOE_Prod_w_Ethane = NETOIL_Bbl + NETGAS_Mcf_Shrunk_w_Ethane / 6 + NETNGL_Bbl_w_Ethane, 
         NETBOE_Prod_wo_Ethane = NETOIL_Bbl + NETGAS_Mcf_Shrunk_wo_Ethane / 6 + NETNGL_Bbl_wo_Ethane) %>% 
  left_join(., fcube %>% filter(ScenarioName == "Budget" | ScenarioName == "Forecast") %>% 
               select(ProductionMonthEndDate, ScenarioName, NetSalesGasMCF, NetSalesOilBBLs,
                      BudgetNode, NetSalesNGLBBLs), by = c("BUDGET_NODE" = "BudgetNode", "MON_END_DATE" = "ProductionMonthEndDate"))
